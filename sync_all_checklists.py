...
def sync_institutional_checklist():
    ...
    try:
        ...
        for item in INSTITUTIONAL_CHECKLIST_ITEMS:
            ...
            # Check by item_id first (preferred), then fall back to item_order
            cursor.execute(f"""
            ...
            if existing:
                cursor.execute(f"""
                    ...
                    WHERE id = {ph}
                """, (item['desc'], item['wt'], 1 if item['critical'] else 0, item_id,
                      existing[0]))
            else:
                cursor.execute(f"""
                    INSERT INTO form_items
                    (form_template_id, item_order, category, description, weight, is_critical, item_id, active)
